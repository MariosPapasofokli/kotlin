...
fun LazyJavaResolverContext.findClassInJava(fqName: FqName): JavaClassLookupResult {
    ...
    if (kotlinClass != null && kotlinClass.getKind() != null) {
        if (kotlinClass.getKind() == KotlinClassHeader.Kind.CLASS) {
        ...
        else if (kotlinClass.getKind() == KotlinClassHeader.Kind.INCOMPATIBLE_ABI_VERSION) {
            errorReporter.reportIncompatibleAbiVersion(kotlinClass, kotlinClass.getAbiVersion())
        }
        else {
